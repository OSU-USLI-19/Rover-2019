...
void transmitShaggy(String sentence){
  ...
  XBeeAddress64 addr64 = XBeeAddress64(ShaggyHigh, ShaggyLow);

  ZBTxRequest MyTX = ZBTxRequest(addr64, packet, sizeof(packet));
  
  xbee.send(MyTX);
}  

String getTransmission(){
  String MyTransmission = "";

  Rx64Response RX64 = Rx64Response();
  xbee.readPacket(100);

  MyTransmission = processPayload(RX64.getData(), RX64.getDataLength());
  /*
  for(int i = 0; i < 80; i++){
    MyTransmission[i] = RX64[i];
  }
  */
