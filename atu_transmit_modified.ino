...
void setup()
{
   Serial3.begin(9600); // Sends to ground station, this needs to be raised
   ...
   if( (index1array[0] == '$') && (index1array[3] == 'R'))
   {
      ...
      */
      ...
      for(i = 0; i < (sentence.length() + 1); i++){
        ...
        bufferPacket[i] = (uint8_t)temp;
      }
      bufferPacket[i - 1] = 0xEE; // Set the terminator

      // packet-up ;D 
      payloadPacket = txRequestPacketGenerator(harold_h, harold_l, bufferPacket);
